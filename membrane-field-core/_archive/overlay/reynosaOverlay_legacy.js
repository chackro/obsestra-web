...
function relocateRestrictedSource(srcIdx) {
    ...
    // Fallback: return original if no safe cell found (don't crash, just use original)
    console.warn(`[RELOCATE] Source relocation failed for ${srcIdx}, using original`);
    return srcIdx;
}
...
export function onAttach(context) {
    ...
    // Init particle layer (visual only - follows nextHop graph)
...
function _dbgLogSourceProbe(externalSimTimeSeconds, simDeltaSeconds, contextLabel = '') {
    ...
    const particles = particleLayer.getParticles() || [];
...
export function onFrame(camera, time, realDeltaSeconds = 1/60) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // PARTICLE UPDATE — TIME AUTHORITY (see field_authority.md)
    // - Birth rate: sim time (physics authority via emitParticlesForPhysicsTime)
    // - Position death (sink): physics authority (world state)
    // - Motion + age: sim time (particles are Lagrangian tracers of the field)
    // ═══════════════════════════════════════════════════════════════════════════
    if (particleLayer && state !== OverlayState.OFF) {
...
function updateMultiClassPhysics(dt) {
    ...
    for (let substep = 0; substep < PHYSICS_SUBSTEPS; substep++) {
        ...
        // no per-lot minting for particles here
...
function graphFlowClass(classId, rho, rhoNext) {
    ...
    for (const idx of roadCellIndices) {
        ...
        const out = m * FLOW_FRAC * congestion;
            ...
            // Rejected mass stays in source cell (queues on road).
            // "No roadside storage" - storage only in lots, but entry is gated.
            // ─────────────────────────────────────────────────────────────
            if (classId === 'restricted' && regionMap[nh] === REGION_LOT) {
                // Prevent overflow-by-oversubscription within a substep:
                // reserve remaining capacity per-lot as we accept inflows.
                const lotIdx = cellToLotIndex[nh];
                const remaining = (lotIdx >= 0 && lotAcceptRemainingKgLive) ? lotAcceptRemainingKgLive[lotIdx] : 0;
                ...
                const acceptMultiplier = getLotAcceptance(nh); // reads lotMassKgLive snapshot (soft shape)
                ...
                if (lotIdx >= 0 && lotCapacityKg.length > 0) {
                    ...
                    if (cells && cells.length > 0) {
                        ...
                        rho_restricted_lot[nh] += accepted;
                    }
...
function emitParticlesForPhysicsTime(dt) {
    ...
    for (const i of sourceCellIndices) {
        ...
        // PARK LOCAL DWELL: For park sources, emit deterministically by class
        // Restricted particles go to park wait zones; cleared particles enter road network
        const parkWaitIdx = _sourceToWaitZoneIdx.get(i);
        if (parkWaitIdx !== undefined) {
            // Park source: split by class and emit deterministically
            const restrictedKg = massKg * TRANSFER_REQUIREMENT_FRACTION;
            const clearedKg = massKg * (1 - TRANSFER_REQUIREMENT_FRACTION);

            // Restricted: emit as parked in park wait zone
            particleLayer.emitFromMass(restrictedKg, i, 'restricted', parkWaitIdx);
            parkRestrictedEmitted += restrictedKg;

            // Cleared: emit to road network normally
            particleLayer.emitFromMass(clearedKg, i, 'cleared', -1);
            parkClearedEmitted += clearedKg;

            totalEmitted += massKg;
        } else {
            // Corridor source: emit with random class assignment (existing behavior)
            particleLayer.emitFromMass(massKg, i);
            totalEmitted += massKg;
        }
    }
    ...
    if (_particleEmitDebugCounter++ % 60 === 0) {
        const accumulators = particleLayer.getAccumulatorsBySource();
        let sampleAcc = 0;
        if (sourceCellIndices.length > 0) {
            sampleAcc = accumulators.get(sourceCellIndices[0]) ?? 0;
        }
        ...
        console.log(`[PARTICLE EMIT] simDt=${dt.toFixed(1)}s emittedKg=${totalEmitted.toFixed(0)} sampleAcc=${sampleAcc.toFixed(0)}/${PARTICLE_MASS_KG}${parkInfo}`);
    }
}
...
function bakeKTensor(geometry) {
    ...
    // If no segments loaded, stamp mock roads for testing
    if (!geometry.roadSegments || geometry.roadSegments.length === 0) {
        console.log('[MOCK] No segments found, stamping mock roads');
        stampMockRoads();
    }
    ...
    initCorridorEntries();
    phiBaseDirty = true;
    console.log('[ReynosaOverlay] K tensor baked from', geometry.roadSegments?.length || 0, 'segments');
}

/**
 * Stamp mock roads for testing particle flow without real segment data.
 * Creates simple paths from entry points to Pharr.
 */
function stampMockRoads() {
    const K_ROAD = 1.0;
    const roadWidth = 3;  // cells

    // Pharr sink location (world coords)
    const pharrX = 0;
    const pharrY = 0;

    // Entry points (world coords)
    const entryEast = { x: -5150, y: -30067 };
    const entryWest = { x: -39274, y: -11176 };

    // Helper to stamp a line between two world points
    function stampLine(x1, y1, x2, y2) {
        const fx1 = worldToFieldX(x1);
        const fy1 = worldToFieldY(y1);
        const fx2 = worldToFieldX(x2);
        const fy2 = worldToFieldY(y2);

        const dist = Math.hypot(fx2 - fx1, fy2 - fy1);
        const steps = Math.ceil(dist);

        for (let i = 0; i <= steps; i++) {
            const t = steps > 0 ? i / steps : 0;
            const fx = Math.floor(fx1 + t * (fx2 - fx1));
            const fy = Math.floor(fy1 + t * (fy2 - fy1));
    ...
    // Road from ENTRY_EAST to Pharr (via intermediate points)
    stampLine(entryEast.x, entryEast.y, entryEast.x, -15000);  // North
    stampLine(entryEast.x, -15000, pharrX, -15000);            // West
    stampLine(pharrX, -15000, pharrX, pharrY);                  // North to Pharr

    // Road from ENTRY_WEST to Pharr (via intermediate points)
    stampLine(entryWest.x, entryWest.y, entryWest.x, -5000);   // North
    stampLine(entryWest.x, -5000, -20000, -5000);              // East
    stampLine(-20000, -5000, -20000, pharrY);                   // North
    stampLine(-20000, pharrY, pharrX, pharrY);                  // East to Pharr

    // Cross road connecting both corridors
    stampLine(-30000, -10000, -10000, -10000);

    console.log('[MOCK] Stamped mock road network from entries to Pharr');
}
...
function updateMetrics(dt) {
    ...
    if (typeof __DEV__ !== 'undefined' && __DEV__ && particleLayer?.getParticles) {
        const particlesCount = particleLayer.getParticles().length;
...
export function setLocalScenario(options) {
    ...
    if (options.particleConfig) {
        ...
        if (particleLayer) {
            particleLayer.configure({ ...localScenario.particleConfig, sourcesDirty: true });
        }
    }
...
export function toggleParticleDebugClassColors() {
    if (particleLayer) {
        return particleLayer.toggleDebugClassColors();
    }
    console.warn('[PARTICLE DEBUG] Particle layer not initialized');
    return false;
}
...
export const ReynosaEastOverlay = {
    ...
    onFrame,
    draw,
    setCitySegments: () => {},  // Stub for compatibility with testBundle
