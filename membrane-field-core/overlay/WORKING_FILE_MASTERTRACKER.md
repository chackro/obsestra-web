...
### 3. Metrics are counts, not kg
```javascript
const metrics = {
    injected: 0,  // COUNT, not kg
    exited: 0,    // COUNT, not kg
```
**Impact:** To get kg: `metrics.injected * TRUCK_KG` (but TRUCK_KG not exported)
...
### 5. No direct step() or reset() export
**Impact:** Cannot run headless physics without going through `update()`.

## VALIDATION CHECKLIST

| Metric | Accessible? | How |
|--------|-------------|-----|
| truckHoursLost | YES | `getMetrics().truckHoursLost` |
| truckHoursLostRate | YES | `getMetrics().truckHoursLostRate` |
| injectedKg | INDIRECT | `getMetrics().injected * TRUCK_KG` (TRUCK_KG = 18000) |
| exitedKg | INDIRECT | `getMetrics().exited * TRUCK_KG` |
| activeParticles | YES | `getParticleCount()` |
...
## MINIMUM CHANGES FOR HEADLESS

To run physics without render:
1. Export `step(dt)`
2. Export `reset()`
3. Remove `_particlesDirty = true` from step() (move to update())
4. Make camera optional in update()
5. Export `TRUCK_KG` constant OR add `injectedKg`/`exitedKg` to getMetrics()
